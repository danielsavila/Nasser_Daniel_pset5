...
def scrape_hhs_oig(year, month):
    ...
    Returns:
        list: A list of dictionaries, each containing the scraped data.
    """

    url = "https://oig.hhs.gov/fraud/enforcement-actions"
    ...
    ul_children = soup.find_all(lambda t: t.name == 'ul' and t.find_all("div"))
    for action in ul_children:
        title = action.find_all("h2")
        date = action.find_all(lambda t: t.name == "div" and t.find_all("span"))
        categories = action.find_all(lambda t: t.name == "div" and t.find_all("ul"))
        link = action.find_all("a")
        enforcement_actions["ea_titles"].add(title.get_text())
        enforcement_actions["dates"].add(date.get_text())
        enforcement_actions["categories"].add(categories.get_text())
        enforcement_actions["links"].add(link.get_text())

    df_enforcements = pd.DataFrame(enforcement_actions)

    return df_enforcements
```

```{python}
df_test = scrape_hhs_oig(2023, 11).head()
```
