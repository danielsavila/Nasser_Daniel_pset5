...
def scrape_hhs_oig(year, month):
    ...
    for i in range(480):
        ...
        # Extract title and link
            ...
            #agency_li = agency_soup.find('li', text=lambda text: text and 'Agency:' in text)
            for li in agency_soup.find_all('div',class_ ='margin-top-5 padding-y-3 border-y-1px border-base-lighter'):
...
#path_districts = os.path.join(filepath,'geo_export_3d06657f-30d6-461b-af00-153ad6e863c9.shp')
path_census = os.path.join(filepath,
 'cb_2018_us_cd116_500k.shp')

#df_districts = gpd.read_file(path_districts)
districts_path = os.path.join(filepath,'US_Attorney_Districts_Shapefile_simplified_20241107.csv')
df_districts = pd.read_csv(districts_path)
df_census = gpd.read_file(path_census)

df_districts["STATEFP"] = df_districts["STATEFP"].astype(str)
df_districts["Judicial District "] = df_districts["Judicial District "].astype(str)
df_merged = df_districts.merge(df_census, on = "STATEFP" ,how = 'left')
```
...
df_merged.plot(column = "Judicial District", linewidth=0.2)
plt.axis("off")
plt.show()
```
