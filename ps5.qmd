...
df_zip_pop['NAME'] = df_zip_pop['NAME'].apply(lambda x: x.replace('ZCTA5 ', ''))
...
```{python}
spatial_joint = gpd.sjoin(df_merged_zips,df_districts,how = 'inner', predicate = 'intersects')
...
```{python}
district_ratio = district_pop.merge(
    df_merged, on ='name',how = 'inner')
district_ratio['ratio'] = round(district_ratio['count']/district_ratio['population'],3)*100 
agg_ratio = district_ratio.groupby('name').agg(geometry = ('geometry_y','first'),ratio = ('ratio','first'))
```

```{python}
fig, ax = plt.subplots(1, 1, figsize=(12, 8))
agg_ratio.plot(column='ratio', ax=ax,)
